...
async function genScriptCode(scriptVar, descriptor, filename, options, _pluginContext) {
    var _a, _b, _c;
...
async function genTemplateRequest(filename, descriptor, _pluginContext) {
...
async function genCustomBlockCode(filename, descriptor, _pluginContext) {
    let code = '';
...
async function genStyleRequest(cssModuleVar, descriptor, filename, _pluginContext) {
    let scoped = false;
    let stylesCode = '';
...
function normalizeDevPath(root, id) {
    ...
    else if (fs_1.default.existsSync(cleanUrl(id)))
